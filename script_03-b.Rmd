...
```{r}
sep_new %>%
inner_join(sep_old, sep_new, by = "id", suffix = c("_old", "_new")) %>%
    mutate(enrollment_change = u_grad_old - u_grad_new) %>%
arrange(desc(enrollment_change)) %>%
  slice_head(n=5) %>%
    select(name_new, department_new, enrollment_change) %>%
   gt() %>%
   tab_header(title = md("**Top 5 Classes with Highest Increase in Enrollment**")) %>%
    cols_label(name_new = "Course Name", 
             department_new = "Department",
              enrollment_change = "Enrollment Increase") %>%
  tab_source_note(md("Source: Harvard University Registrars' Office"))
```
...
arrange(desc(enrollment_change)) %>%
  slice_head(n=5) %>%
    select(name_new, department_new, enrollment_change) %>%
   gt() %>%
   tab_header(title = md("**Top 5 Classes with Highest Decrease in Enrollment**")) %>%
    cols_label(name_new = "Course Name", 
             department_new = "Department",
              enrollment_change = "Enrollment Decrease") %>%
  tab_source_note(md("Source: Harvard University Registrars' Office"))
```
...
arrange(desc(enrollment_change)) %>%
  slice_head(n=5) %>%
  mutate(enrollment_change = -(enrollment_change)) %>%
    select(name_new, department_new, enrollment_change)
...
full_join(increase, decrease, by = c("name_new", "department_new", "enrollment_change")) %>%
  arrange(desc(enrollment_change)) %>%  
  ggplot(aes(x=name_new, y=enrollment_change))+
  geom_col() + 
  coord_flip() +
  labs(Title = "Fall 2020 Classes with Biggest Changes in Enrolment", x = "Course Name", y= "Change in Enrolment") 
```
...
# The number of hours of work in the first few weeks of class - the qguide only gives us the average number of hours over the course of the semester.
# Whether a course with less hours of work would have fewer numbers of students dropping out (Fundamental Problem of Casual Inference).
